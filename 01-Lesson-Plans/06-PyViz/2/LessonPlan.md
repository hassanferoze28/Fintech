## 6.2 Lesson Plan: Customized Visualizations and Data Aggregation

### Overview

Today’s class will focus on building professional and compelling visualizations. Students will continue where they left off in the last lesson by customizing visualizations by using the different options that hvPlot provides. Students will also explore techniques for data aggregation, first by using the Pandas `groupby` function to prepare data for visualizations, then by using visualizations.

### Class Objectives

By the end of this class, students will be able to:

* Customize visualizations by using hvPlot attributes and options

* Aggregate data by using the Pandas `groupby` function

* Explore data by using visual aggregation techniques

* Tell a story by using custom visualizations

---

### Instructor Notes

* This lesson encourages students to professionalize their plots by adding details such as axis labels and colors. Be sure to share your perspective on what makes a visualization effective and clear. Stories and anecdotes from your work experience can help emphasize why these details matter.

* Students will also use Pandas to prepare data for visualizations in this lesson. Although students will have some experience with data aggregation from previous lessons, their comfort level with this will vary. Make sure to provide opportunities for students to ask questions about these aggregation techniques.

* The lesson ends with an activity in which students can create their own custom visualizations and use them to tell stories. Encourage students to be creative with this activity! 
...
In this section, you will provide an overview of today’s lesson. Start by welcoming students to their second day of visualization. Review the class objectives, and explain the focus of the class.

* In the last class, you learned how to use hvPlot to make visualizations. Today we’re going to build on that work to create more complex and more professional visualizations.

* Visualizations serve many purposes in our work. Sometimes we may use one line of code to make a quick plot to explore our data. That’s fine, but if we’re making a plot to share with others, it’s important to make it as clear and easy to interpret as possible.

* Today we’ll learn how to add titles, axes, legends, colors, and other details to our plots. These are all important elements that you will need to use any time you create a professional visualization.

* We’ll also work on **data aggregation**, which is the process of preparing and summarizing data. We’ll do this two ways: by using the Pandas `groupby` function, and by using visualization tools to aggregate data.

* At the end of class, you’ll use these tools to create a visualization that you can use to tell a story. Visualizations are important communication tools, so we’ll talk about strategies for data storytelling with visualizations.

---

### 2. Instructor Do: Visualization Options (15 min)

**Corresponding Activity:** [01-Ins_Viz_Options](Activities/01-Ins_Viz_Options)

In this section, you’ll demonstrate how to use hvPlot attributes and options in order to customize the look and feel of visualizations. Students will learn how to add detail to their visualizations, such as axis labels, color themes, and effects.
...
* Without customization, visualizations of large datasets often have axis labels that are difficult to read. The `rot` attribute allows users to customize label angles in order to make labels more readable. The `rot` attribute accepts an integer value that indicates the angle at which x-axis labels should be rotated, as shown in the following code.
...
* You can add a title to a visualization by using the `opts(title)` parameter.

* You can also add a title to the plot with a label parameter, as the following code shows. A title added in this manner is reflected in the legend of a compose or overlay plot.
...
Highlight the `color` and `hover_color` options, as shown in the following code. Explain that `hover_color` is especially useful in a bar plot with a lot of bars.
...
In this activity, students will use hvPlot customization attributes and options to add details to their visualizations.
...
Be sure to slack out the [hvPlot customization](https://hvplot.pyviz.org/user_guide/Customization.html) documentation so that students have a complete list of all options available.
...
In this section, the class will review the solution to the previous activity, and students will present their final, customized visualizations.

**Files:**

* [picture_perfect.ipynb](Activities/02-Stu_Picture_Perfect/Solved/picture_perfect.ipynb)

Ask for volunteers to demo their customized visualizations. Encourage students to share any tricks or shortcuts they learned in the process of building their visualization, as well as give constructive criticism to their peers regarding how to make their visualizations more appealing and insightful.

When students present, ask them how they used the `opts` function and other customization attributes to perfect their visualizations.

Review the solution to the activity by highlighting the following points:

* The code for the stylized plot for the revenue figures for the United States and Canada region is as follows:
...
* The code for the stylized plot for the revenue figures for Europe, Middle East, and Africa is as follows:
...
* The code for the composite plot for these two visualizations is as follows:
...
Here is the composite visualization:

![overlay_styled.png](Images/overlay_styled.png)

If time remains, ask the following review questions:

* What are the different types of customizations we covered today?

  * **Answer:** Labels, axes, and color.

* What is the argument of the `rot` function?

  * **Answer:** The angle of the x-axis labels. This could be 90, 45, and so on.

* Can composite plots be customized?

  * **Answer:** Yes. Composite plots can be customized with the `opts` function. The composing operation should be placed within parentheses in order for the `opts` function to work.
...
* Now that we’ve made some beautiful, professional plots, let’s switch gears and talk about **data aggregation**, which is often the first step to creating a visualization.
...
* Working in fintech, you’ll need to do analyses that rely on the relationships among multiple variables. For example, a real-estate analyst determines a property’s sale or rental price based on various factors: the square footage, number of rooms, property size, neighborhood, school district, taxes, and more. 
...
* One powerful technique for exploring and understanding data relationships is aggregation. We already used aggregation to summarize data in previous lessons. 

Now ask the students to recall the types of aggregation they have done in the past:

* What techniques have we used to describe or summarize data in the past?

  * **Answer:** Functions like `mean`, `count`, `sum`, `min`, and `max`. Each of these functions can summarize, or **aggregate**, a sequence of values. 
...
* **Numerical aggregation** involves evaluating a group of numbers to compile and summarize information about the group. 

* For example, we have used the Pandas `describe` function to summarize a DataFrame. The `describe` function uses many aggregates, such as the mean, count, and minimum and maximum values. Each of these aggregates give us new ways to summarize and display the data. 
...
* We can also directly use aggregate functions, such as `mean` and `sum`, to summarize our data:
...
* Say that we want to estimate the average housing price for each zip code. One way to do this is to filter out all the other zip codes and then calculate the summary statistics for just the one neighborhood. The following code shows doing this for zip code 90210:
...
* hvPlot helps us automatically visualize groups with widgets. **Widgets** are functional elements in a plot that allow us to easily change the visualization. This way, we can highlight how each group affects the overall plot, the relationships among different groups, and how the analysis might change if we change the groups. 
...
* Because a zip code is a series of numbers, hvPlot selected the slider widget. Because a state abbreviation has a different data type, it selected a dropdown menu. It automatically customizes the plot and widget type for the data that we’re working with. However, options exist for overriding its selections. 
...
* **Answer:** We can see from the interactive visualization that there are some average total payments that stand above the majority. 
...
### 12. Student Do: Storytelling with Custom Visualizations (20 min)

In this activity, Students will enhance a basic plot by using the visualization options that the hvPlot library makes available.

**Corresponding Activity:** [06-Stu_Storytelling_with_Custom_Visualizations](Activities/06-Stu_Storytelling_with_Custom_Visualizations)

In this activity, you’ll explore medical payments for diabetes by state. You’ll do so by using interactive visualizations, numerical and visual aggregation, and analysis.

**File:** [storytelling_with_custom_visualizations.ipynb](Activities/06-Stu_Storytelling_with_Custom_Visualizations/Unsolved/storytelling_with_custom_visualizations.ipynb)

**Instructions:** [README.md](Activities/06-Stu_Storytelling_with_Custom_Visualizations/README.md)


---

### 13. Instructor Do: Storytelling with Custom Visualizations Review (10 min)

Open [storytelling_with_custom_visualizations.ipynb](Activities/06-Stu_Storytelling_with_Custom_Visualizations/Solved/storytelling_with_custom_visualizations.ipynb) in the Solved folder and review the solution with the students. As you walk through the different sections of the activity, call on students to answer the questions that they should have answered as they worked through the steps of the activity. 

Cover the following points as you walk through the solution file:

* Our process for creating visualizations should be familiar to you now. We start by importing the data, looking at it to confirm the import was successful, and then using the `.loc` function, or any method of your choice, to filter the dataset to values of interest. 
...
Walk through the different steps in the solution to customize the plots. Ask students if they did anything differently in their customization. Then, ask students to share their version of the data story they found for the tread of the real-estate commission. 

If no one volunteers, you can share this story:

* Based on the information depicted in the visualizations, the trend for real estate commissions for 2019 is not great. The average sales per date in March is definitely down from that seen in January and February.
...
* Visualizations are an important part of working in any business that uses data. They are a critical way to summarize information to share with others.

* If a visualization can’t be understood by others, it’s not worth sharing. Make sure to label and style your plots. Even if you do not need to share them with anyone, this will be good practice for you when you return to your work.
