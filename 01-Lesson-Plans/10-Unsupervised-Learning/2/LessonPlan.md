...
In this class, the students will explore machine learning more deeply. The lesson begins with a recap from the last lesson, and then focuses on normalizing, preprocessing, and segmentation. The goal is to enable students to refine their unsupervised learning algorithms and to flexibly apply them to a variety of financial applications.
...
* Welcome to the second day of machine learning! You will guide the students through a series of activities to build on what they know and can accomplish with K-nearest neighbors, their first unsupervised learning algorithm.

* Today's class will stretch the students’ ability to cluster algorithms by teaching them about necessary and commonly used practices in unsupervised learning, like standardizing variables or dealing with categorical (non-numerical) data. They'll also be exposed to clustering methods outside of K-means.
...
* **Question:** What is the difference between unsupervised and supervised learning?
  ...
  * **Sample answer:** The main distinction between the two approaches is the use of labeled datasets.

  * **Sample answer:** Supervised learning uses labeled input and output data, while an unsupervised learning algorithm only has labeled input data.
...
As you work through the solution to the previous activity, be sure to emphasize the following points:
...
* For this activity, the values of 3 and 4 are most likely the optimal values of k. The following code produces the visualizations for each value:
  ...
  ```
The following code produces the visualizations for 4 clusters:
...
* However, 4 clusters is probably not wrong either. This is because for a certain range of inventory, it appears to identify two unique clusters according to the number of homes sold. Overall, the best differentiation across multiple variables would likely be 3 clusters.
...
### 4. Instructor Do: Preparing Data by Normalizing It (20 minutes)

In this section, you’ll introduce the students to normalizing data and then do a brief coding demo.
...
* Data clustering can be optimized by selecting the best value for k. Using the K-means algorithm is useful for grouping and understanding financial data.

* Machine learning algorithms can often be enhanced and optimized by applying principal component analysis, or PCA. PCA is an important optimization technique widely used in the world of quantitative finance, particularly for portfolio construction.
...
* In the Challenge, you’ll manage data and optimize the clusters with PCA, and then analyze the results. Then, you’ll propose the best number of cryptocurrency clusters for the portfolio.
...
In the next few talking points, refer to the “Normalizing Data” slides as you introduce the class to the process of preparing data by normalizing it.

* Preparing data can be time consuming if we do it manually. This is especially true if we have several columns in a DataFrame to normalize, or transform.
...
* When we **normalize** data, we eliminate the measurement units and scale the numeric values to a similar scale. We can then compare data of differing natures.
...
* The most common way to normalize data is to apply **standard scaling**, which is a method of centering values around the mean. Let's review one of our credit card spending datasets to illustrate how standard scaling works.
...
* **Data standardization**, or **data normalization**, is a common practice in the data preprocessing that occurs before training a machine learning model.

Slack out the [scikit-learn documentation on preprocessing and Standardization](https://scikit-learn.org/stable/modules/preprocessing.html#preprocessing-scaler) so the students can learn more about data standardization. Encourage them to refer to the [scikit-learn user guide](https://scikit-learn.org/stable/user_guide.html). 
...
* The columns with data in string or category-type format will need to be handled differently. These `object` types of variables aren't easily standardized. The variable "Card Type", for example, consists of data in a **binary** (that is, "yes" or "no") type of categorical variable.
...
Slack out [this link on Pandas' `get_dummies` function](https://pandas.pydata.org/docs/reference/api/pandas.get_dummies.html) as you elaborate on its usage with the following talking points.
  ...
  * The `get_dummies` function converts categorical data into a numerical format that machine learning models recognize. To do that, the Pandas `get_dummies` function transforms a categorical column of data into multiple columns of separate data, where each entry is a `1` or `0` representation.
...
* Ask the students if they can identify any preprocessing that needs to be done on the `ccinfo_df` DataFrame before a machine learning algorithm can be applied. Potential answers include the following:
  ...
  * "limit_bal", "bill_amt", and "pay_amt" should all be normalized so that they are on the same scale.
...
Explain that the final step of data preprocessing is to **normalize**, **scale**, or **standardize** (which in this case all mean the same thing) the data contained in the "limit_bal", "bill_amt", and "pay_amt" columns.

Remind the students that data columns in a DataFrame need to be normalized when they're in different scales.

* Data is normalized when the values in the column have a **mean of 0** and a **standard deviation of 1**. This same scale is applied to all of the columns that need to be normalized.
...
* It is possible to normalize, or scale, each of these columns manually, but it is much easier to apply the `StandardScaler` module from the scikit-learn library.
...
Then, explain that the next step is to call the `StandardScaler` and the `fit_transform` function on the columns from the DataFrame that need to be normalized.
...
Explain that we’ll now create a dataset to visualize clustering and classification algorithms. Specifically, `make_moons` will create a complex, crescent-shaped dataset with two columns and 500 data points.
...
* Regardless of whether you are participating in a Kaggle contest or working as a consultant in the fintech industry, providing preprocessed and anonymized data is a common practice to share real-world data while protecting  privacy. This is the type of scenario that we want to provide you with for this activity.
...
* Note that the data contains 10 features labeled as `feature_1` to `feature_10`. For companies that share data in this way, there is a feature mapping each anonymous column. As a fintech professional, you will add a meaningful value to the data by applying machine learning and results from interpretation.
...
* Next, we can use hvPlot to visualize these clusters by creating a scatter plot, using the `feature_1` and `feature_2` columns and colouring the data points by setting `by="customer_segments"`. To facilitate comparison, we'll do this for each of the three algorithms, as shown in the following code:
...
Explain to students that while it's often difficult to articulate why a particular observation fits in one cluster or another. Plotting cluster labels against various features in this way helps us to identify more qualitatively the way groups are forming.
  ...
  * For instance, a distinct cluster (labeled as `2`) is found for all three algorithms, in which observations in this cluster tend to have high values of `feature_2` and simultaneously low values of `feature_1`.

  * This data on customers has been anonymized and standardized, but if we had the original variable names, we might be able to communicate with non-technical members of the bank about why, intuitively, our algorithm is categorizing a customer with a particular label.
