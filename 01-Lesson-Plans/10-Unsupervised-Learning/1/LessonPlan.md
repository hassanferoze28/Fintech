...
* Machine learning algorithms learn how to make decisions without needing anyone to program all that logic. They learn the patterns, behavior, and logic on their own directly from the data and then use that knowledge to make decisions and predictions.
...
* The use of machine learning in fintech is vast. These include identifying money laundering and legal violations; recognizing satellites for the real-time awareness of trading opportunities in commodities; and predicting customer churn in financial products.
...
* **Supervised learning**: The algorithm learns on a labeled dataset, where each example in the dataset is tagged with the answer. This provides an answer key that can be used to evaluate accuracy of the training data.
...
* Compare unsupervised learning to supervised learning by explaining that:
  ...
  * Supervised learning uses labeled input data, and unsupervised learning takes unlabeled data.
...
* **Question:** How might clustering be used by fintech businesses?
  ...
  * **Answer:** One possible answer is that it can be used to group customers by spending habits and create customized offers via email or mobile apps.
...
Highlight the parameter `random_state=1`, which ensures that code will return the same randomized data set each time it is run; this ensures the same output every time. 
...
Explain that the process of clustering data points into groups is called centering.
...
* The students should apply a different number of `center` points to the same `random_state` variable, and the same number of center points to different `random_state` variables in order to compare and contrast the results. The following are examples of different permutations and combinations:
...
* The K-means algorithm works by performing two steps:
  ...
  1. Assigns points to the closest cluster center.

  2. Re-adjusts the cluster's center by setting each center as the mean of all the data points contained within that cluster.
...
* With two groups, it looks like there is a group that favors using the mobile application and a group that prefers to use an in-person banker to help with their financial transactions.
...
* Create the K-means model for four clusters. Emphasize the role that the `random_state` parameter plays in ensuring that all of the groups get the same result.
...
* Add the customer segmentation information to the `service_ratings_predictions_df` DataFrame. This allows us to visualize the customer segmentation data in relation to the two columns of rating information.
...
### 9. Instructor Do: Introduction to Clustering Optimization (5 minutes)

Refer to the “Introduction to Clustering Optimization” slides as you discuss the following points about clustering optimization: 
...
* Explain that the following image shows the updated DataFrame with an index, a column labeled “k” that shows the number of clusters, and a column labeled “inertia” that shows the inertia values for each number of clusters.
...
* For this activity, the values of 3 and 4 are most likely the optimal values of k. Here is the code and visualizations for each value.
...
* You can use the K-means algorithm to identify clusters in a given dataset, and how to optimize this algorithm by using the elbow method.
