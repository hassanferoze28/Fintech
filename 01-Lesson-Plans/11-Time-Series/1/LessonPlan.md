...
* Be sure to emphasize the role of machine learning in time series analysis and **advanced analytics** (the application of advanced statistical models and intelligent algorithms).
...
* To summarize, we use machine learning when we develop a statistical or algorithmic model of existing data that can automatically make predictions or decisions about new data.
...
* Time impacts not only our behavior but also how the financial world works. For example, when you make an investment or deposit money in your savings account, the value of the money can increase over time.

* Time is so important in finance that we need to learn how to manipulate, analyze, and understand data that’s measured over time. This type of data is called **time series data**.

Explain that, in this class, the students will learn the details of manipulating time series data with Python and Pandas. They will learn to use these tools' advanced capabilities for analyzing and working with data in multiple formats from different sources. With these skills, students can build more sophisticated time series models, including ones that can make predictions across different time scales, such as days, weeks, and years, or time zones.
...
Open the Challenge instructions, then go through the steps while emphasizing the following points:

* Advanced analytics is becoming very popular in the fintech industry. It offers developers and companies a way to analyze thousands of large datasets and to use computer predictions that drive investments and decision making.
...
* In today's lesson, you’ll process and analyze time series data by using Python and Pandas.
...
In this demo, we'll analyze the [S&P 500](https://en.wikipedia.org/wiki/S%26P_500), which is the index of the top 500 public stocks in the United States.
...
In this activity, the students will learn how to analyze data across time by using advanced slicing techniques and time pattern identification.

Explain to the students that you’ll demonstrate Pandas slicing functionality to analyze date data and search for specific days, months, minutes, or hours within a DataFrame.
...
* Now, we can start using our new `DatetimeIndex` to analyze the data across time with Pandas slicing features.
...
* We'll slice data by the hour to visualize trends that occur over different hourly periods. Visualizing time series data can be a powerful first step toward identifying patterns.
...
* In the stock market, patterns of behavior often result from everyday human activities and behaviors. Examples include going to lunch or feeling excited about the market opening or closing.
...
* Keeping these patterns in mind, let's try to find some of them in our S&P 500 data by visualizing them with some plots!
...
* Now, we can choose a date to analyze in more detail. Let’s choose `2019-06-06` because, as we’ll find out, this date demonstrates some of the common trading patterns that we mentioned earlier.
...
* Because we have already set the `DatetimeIndex`, we can use the `loc` function to choose the day that we want to analyze. We simply pass it a date or a `datetime`.
...
* We can take any data that has time-related elements, tell Pandas its time data, start slicing, and then start analyzing.

* As you practice applying real-world knowledge to their data, you will recognize how to use these techniques in different contexts.
...
* Next, we use the Pandas `plot` function to visualize the closing price of the TSLA stock:
...
* Finally, we can zoom in even further to analyze the TSLA closing prices from August 22 to September 5, when the price had its largest decrease. We'll use the `loc` function to plot a time slice at a daily level of detail, as the following code shows:
...
Explain to the students that one of the goals of time series analysis is to support better decision making by understanding the behavior of time series events. Give the following example:

* For example, by using home sales data, we can identify the best time to sell a property based on increasing demand or higher sale prices. To make such decisions, we need to identify any pattern that exists in a visualization that depicts financial data over time.

Explain that, in this section, we’ll create and interpret visualizations, such as line plots, to analyze time series data for decision making. We’ll also analyze time series data by using heatmaps to recognize relationships.
...
* To analyze the time series data in chronological order, we next use the Pandas `sort_index` function to order the data in ascending order by the `DateTime` index, as the following code shows:
...
Continue the demo, and explain to the students that you’ll now analyze the homes sold series from a quarterly perspective. You’ll use the `DatetimeIndex` attributes and the `groupby` function to compute the total home sales per quarter, as the following code shows:
...
* For example, we can drill down from analyzing yearly home sales to analyzing quarterly home sales per year, as the following code shows:
...
Explain that grouping time series data and creating line plots offers a visual method for analyzing time series and identifying trends. But, depending on the patterns that we want to explore, we might want to use other types of visualizations, such as scatter plots, heatmaps, or histograms.
...
* The hvPlot library offers an interesting alternative for visualizing seasonal patterns: heatmaps. Let’s explore the quarterly sales per year from this new perspective.
...
* The number of homes sold during the year and in each quarter are represented by the colors associated with each of the boxes at the intersection of the axes. The index for the colors associated with the heatmap is located on the right side of the visualization. The lighter the color, the fewer homes that were sold. The darker the color, the more homes that were sold.
...
* **Question:** We can observe that the third quarter has the highest sales activity. But, has this seasonal effect grown stronger or weaker over recent years?
  ...
  * **Possible Answer:** The seasonal effect on home sales grew progressively stronger over each third quarter. We can observe this in the color of the blocks associated with the third quarter of each year. The blocks get progressively darker (from left to right) along the x-axis.

Elaborate on the answer:

* Our answer to this question illustrates why visualizing seasonal effects can be useful. By examining the heatmap, we can observe that the third quarter always had lots of real estate activity relative to the rest of the year.

* But in recent years, this quarterly effect is more pronounced. The third quarter of 2020, for example, shows an unprecedented amount of seasonal sales activity. (We can observe this in the heatmap by the dark blue coloring for this period). If we notice an increasing trend in this third-quarter effect, that insight will prove useful for making future predictions.

Answer any questions before moving on.

---

### 11. Student Do: Analyzing Time Series Patterns in the S&P 500 Index (20 minutes)
...
In this activity, the students will use their newly developed skills to visualize and analyze time-series patterns in the S&P 500 volume data.
...
* Next, we use hvPlot to visualize the hourly trends across the days of week in a heatmap, as the following code shows:
...
Before moving on, ask the students if they have any questions about hvPlot or the process of analyzing the visual output of time series data.
