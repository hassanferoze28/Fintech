...
* What if an error occurs when you test your Amazon Lex bot after enabling Lambda initialization and validation in the bot? You won't get much information in the Amazon Lex console. In most cases, the error results from a typo in an intent or slot name. Logic errors can also occur in the Python code. So, you can review the code and test the function in the Lambda console for debugging.
...
Explain that we’ll use the `lambda_handler` function to route the incoming requests that are based on the user intents that the Amazon Lex bot captures. So, they'll now learn how to organize the code of a Lambda function to connect with Amazon Lex.
...
Explain that as the preceding image shows, we can organize a Lambda function into six general building blocks. These will manage the interaction with Amazon Lex and implement the necessary business logic.
...
4. **Intents Handlers:** Implements the functionality to fulfill the user's intents. The core business logic is coded into the intents handlers. An **intent handler** is a function that implements the functionality to fulfill a user's intent.
...
* The event structure depends on the service that we connect to Lambda. In this demo, we’ll learn how to define a test event that fulfills the intent of an Amazon Lex bot.
...
In this activity, you’ll do a final recap with the class. Use the lesson slides to summarize what the students learned in today's class:

* As we learned today, we can build complex and automatic dialogues with just a few clicks by using Lambda functions.

* We can use Lambda functions to improve the NLP experience for users by both debugging and adding customized business logic.

Congratulate the students on gaining a new valuable skill! Now, they can create and deploy a serverless application by using Lambda. They can also add customized business logic and data validation by making a connection to Amazon Lex.

Mention that CUIs offer several opportunities to engage with customers in a new way. And, a chatbot offers a virtual agent that can support customers of financial organizations 24-7. By adding rules with Lambda, the students can now create a bot that will help customers by answering questions in the form of natural language.
