...
* Today's class will include a tour of JupyterLab. There will be several live-coding activities that should be completed using the JupyterLab IDE but **not** a Notebook. Students will not begin using Notebooks for running their code until Week 3. Make sure JupyterLab is installed and running properly on your machine prior to class.
...
### Traditional Python files vs Jupyter Notebooks

* Highlight to students that some IDEs have their own file extensions instead of the traditional file endings of the respective programming language they were created for. That is, while traditional Python files have the **.py** ending, Jupyter notebooks have their own file format with the ending **ipynb**, which is what we will use most frequently throughout the course. 
...
* JupyterLab supports multiple file formats, such as .py, .ipynb, .md, and .txt. It also supports editing of CSV, JSON, and markdown files.
...
* Open up the file 'hello_world.py', then open a terminal inside of Jupyterlab as illustrated below. Run the file from the command line (make sure you are in the same folder as the file) by running:
  ...
  ![execute_python_file.PNG](Images/execute_python_file.PNG)
