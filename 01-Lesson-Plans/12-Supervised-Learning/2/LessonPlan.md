...
In this class, the students will gain hands-on experience with random forests and ensemble methods such as bagging and boosting. They will also learn the benefits of using tree-based algorithms for classifications problems.
...
* Today, the students will learn about several new machine learning models, such as support vector machines (SVM), k-nearest neighbors, and random forests.
...
* We will use a synthetic dataset generated by `make_blobs()`. Then, we’ll plot the features of this synthetic data in a scatter plot, varying the color of each observation according to its `y` label value.
...
* One interesting way to analyze a decision tree is to visualize it. We import the following libraries to create a visual representation of the decision tree.
...
* Students should have already installed the [`pydotplus` package](https://anaconda.org/conda-forge/pydotplus). If not, they can install it in their virtual environments by executing the following command in the terminal. (Students who are having issues with the installation can skip this visualization step.)
...
An interesting way to analyze a decision tree is to visualize it. We'll use the `pydotplus` library to create a visual representation of the final decision tree.
...
Explain to students that the next session will cover the use of other tree algorithms, such as ensemble learners, that are better at modeling imbalanced classes.
...
Use the unsolved Jupyter notebook to live-code the solution while highlight the following details.
...
* When we create the random forest instance, we need to set two important parameters:
  ...
  * `n_estimators`: This tells the algorithm how many random forests to create. In general, a larger number makes the predictions stronger and more stable. However, a very large number can result in longer training time. A good approach is to start low and increase the number if the model performance isn't adequate.
    ...
    * A [research study](https://doi.org/10.1007/978-3-642-31537-4_13) suggests that we might use a range between `64` and `128` trees in a forest for initial modeling.
...
* If we need to drop some features, analyzing feature importance could help us to decide which features can be removed.
...
In this activity, the students will explore how to use the random forest algorithm for identifying fraudulent loan applications. Students will use the `sba_loans_encoded.csv` file that they created previously to train the model.
...
### 12. Instructor Do: K-Nearest Neighbors (10 minutes)
...
In this section, you will introduce the **k-nearest neighbors** (KNN) algorithm for the purpose of supervised learning. This final activity for the day is part review and part new material. Highlight the following points:
...
* However, we also can use k-nearest neighbors for supervised learning (data with labeled outcomes). k-nearest neighbors can categorize observations and choose class labels for data that are most similar to each other.
...
* The `KNeighborsClassifier` model accepts the parameter `n_neighbors` which defines how many neighbors should be used in the averaging calculation. This is similar to setting the value of lowercase-k, where `k` identifies the number of segments in the dataset.
...
**Answer:** We can count the number of times it predicted each one of the labels right or wrong. A confusion matrix summarizes these counts in a table. We can also evaluate the accuracy, precision and recall using the classification report.
...
* Using the model-fit-predict pattern, we create an instance of the `KNeighborsClassifier` module with an `n_neighbors` value of 3. Explain that setting the value of n-neighbors is similar to setting the value of lowercase-k in unsupervised learning. Next, we fit the model using the `X_train_scaled` and `y_train` data. Finally, we generate the predicted values. The classification report helps us analyze how well the model performed.
